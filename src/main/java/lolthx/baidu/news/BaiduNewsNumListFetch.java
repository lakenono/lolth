...
@Slf4j
public class BaiduNewsNumListFetch extends DistributedParser{
	...
	public void parse(String result, Task task) throws Exception {
		...
		while (true){
			...
			try {
				...
				task.setExtra(extra);
				Task newTask = buildTask(url, "baidu_news_iterator_list", task);
				Queue.push(newTask);
				...
				e.printStackTrace();
				log.error("handle baidu news inner error : {}",e.getMessage(),e,task.getExtra());
				continue;
			}
		}
	}
